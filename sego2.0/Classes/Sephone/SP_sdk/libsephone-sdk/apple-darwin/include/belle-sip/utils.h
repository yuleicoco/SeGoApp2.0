...
typedef enum {
	BELLE_SIP_LOG_TRACE=0, // 将trace调为第1级
	BELLE_SIP_LOG_DEBUG=1,
	BELLE_SIP_LOG_MESSAGE=2,
	BELLE_SIP_LOG_WARNING=3,
	BELLE_SIP_LOG_ERROR=4,
	BELLE_SIP_LOG_FATAL=5,
	BELLE_SIP_LOG_LOGLEV_END=6
} belle_sip_log_level;


typedef void (*belle_sip_log_function_t)(int lev, const char *fmt, va_list args);
...
#define belle_sip_log_level_enabled(level)   (belle_sip_get_log_level() >= (level))

#ifdef HAVE_SLOG3

//#ifdef BELLE_SIP_DEBUG_MODE
#define belle_sip_debug(...)	DebugLogG(__FILE__, __LINE__, __VA_ARGS__)
//#else // BELLE_SIP_DEBUG_MODE
//#define belle_sip_debug(...)
//#endif // BELLE_SIP_DEBUG_MODE

//#ifdef BELLE_SIP_NOMESSAGE_MODE
//#define belle_sip_log(...)
//#define belle_sip_message(...)
//#define belle_sip_warning(...)
//#else // BELLE_SIP_NOMESSAGE_MODE
#define belle_sip_log(_log_level_, ...)		WriteLogG(__FILE__, __LINE__, _log_level_, __VA_ARGS__)
#define belle_sip_message(...)	InfoLogG(__FILE__, __LINE__, __VA_ARGS__ )
#define belle_sip_warning(...)	WarnLogG(__FILE__, __LINE__, __VA_ARGS__ )
//#endif // BELLE_SIP_NOMESSAGE_MODE

#define belle_sip_error(...)	ErrorLogG(__FILE__, __LINE__, __VA_ARGS__)
#define belle_sip_fatal(...)	FatalLogG(__FILE__, __LINE__, __VA_ARGS__)
...
BELLESIP_EXPORT void belle_sip_init_log_file(char *fpath);
...
BELLESIP_EXPORT int BELLE_SIP_CHECK_FORMAT_ARGS(4,5) belle_sip_snprintf(char *buff, size_t buff_size, size_t *offset, const char *fmt, ...);
BELLESIP_EXPORT int belle_sip_snprintf_valist(char *buff, size_t buff_size, size_t *offset, const char *fmt, va_list args);
BELLESIP_EXPORT int BELLE_SIP_CHECK_FORMAT_ARGS(5,6) belle_sip_hex_snprintf(char *buff, size_t bufflen, char *data, size_t datalen, const char *fmt, ...);

BELLESIP_EXPORT void belle_sip_set_log_level(int level);
BELLESIP_EXPORT int belle_sip_get_log_level(void);
