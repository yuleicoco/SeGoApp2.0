...
struct _MediaStream {
	...
	ms_mutex_t lock; // 保护下面的数组
	MSList* smsg_retry_list; // stun消息补发数组
	MSList* smsg_done_list; // stun消息完成数组
};
...
MS2_PUBLIC int media_stream_send_stun_message(MediaStream *stream, const MSStunMsg *smsg);
...
// 停止媒体流的ticker线程
MS2_PUBLIC void media_stream_stop_ticker(MediaStream *stream);
...
MS2_PUBLIC AudioStream *audio_stream_start (RtpProfile * prof, int locport, const char *remip,
				int remport, int payload_type, int jitt_comp, bool_t echo_cancel, MSStunMsg *stunmsg);

MS2_PUBLIC AudioStream *audio_stream_start_with_sndcards(RtpProfile * prof, int locport, 
				const char *remip4, int remport, int payload_type, int jitt_comp, 
				MSSndCard *playcard, MSSndCard *captcard, bool_t echocancel, MSStunMsg *stunmsg);


MS2_PUBLIC int audio_stream_start_with_files (AudioStream * stream, RtpProfile * prof,
				const char *remip, int remport, int rem_rtcp_port, int pt, int jitt_comp, 
				const char * infile,  const char * outfile, MSStunMsg *stunmsg);
 ...
 * @param usermsg send stun user message first.
...
MS2_PUBLIC int audio_stream_start_full(AudioStream *stream, RtpProfile *profile, const char *rem_rtp_ip,int rem_rtp_port,
	...
	MSSndCard *playcard, MSSndCard *captcard, bool_t use_ec, MSStunMsg *stunmsg);
...
**/
MS2_PUBLIC int audio_stream_start_now(AudioStream * stream, RtpProfile * prof,  const char *remip, int remport, int rem_rtcp_port, int payload_type, int jitt_comp,MSSndCard *playcard, MSSndCard *captcard, bool_t echo_cancel, MSStunMsg *stunmsg);
...
/**
 *  send stun user text message
 *  */
MS2_PUBLIC int audio_stream_send_user_message(AudioStream * stream, const char *message);
...
struct _VideoStream
{
	...
	MSFilter *output2;
	MSFilter *tee3;
	// 录制视频只能在本地视频与远端视频两者之中选其一。与音频不同，可以混音至一个过滤器。
	// 配置项，不可随意修改。
	bool_t local_itc; // 是否为录制本地视频
	MSFilter *itcsink;
...
MS2_PUBLIC int video_stream_start_with_source(VideoStream *stream, RtpProfile *profile, const char *rem_rtp_ip, int rem_rtp_port,
		const char *rem_rtcp_ip, int rem_rtcp_port, int payload, int jitt_comp, MSWebCam* cam, MSFilter* source, MSStunMsg *stunmsg);
MS2_PUBLIC int video_stream_start(VideoStream * stream, RtpProfile *profile, const char *rem_rtp_ip, int rem_rtp_port, const char *rem_rtcp_ip, int rem_rtcp_port,
		int payload, int jitt_comp, MSWebCam *device, MSStunMsg *stunmsg);
...
/*returns true if we are still receiving some data from remote end in the last timeout seconds*/
MS2_PUBLIC bool_t video_stream_alive(VideoStream *stream, int timeout);
...
MS2_PUBLIC void video_stream_enable_dynamic_framerate_control(VideoStream *stream, bool_t enable);
...
MS2_PUBLIC int video_stream_recv_only_start(VideoStream *videostream, RtpProfile *profile, const char *addr, int port, int used_pt, int jitt_comp, MSStunMsg *stunmsg);
MS2_PUBLIC int video_stream_send_only_start(VideoStream *videostream,
				...
				int used_pt, int  jitt_comp, MSWebCam *device, MSStunMsg *stunmsg);
