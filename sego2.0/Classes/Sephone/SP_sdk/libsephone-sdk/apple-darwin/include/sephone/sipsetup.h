...
struct _SipSetupContext{
	...
	struct _SephoneProxyConfig *cfg;
	char domain[128];
	char username[128];
	void *data;
};
...
SEPHONE_PUBLIC void buddy_lookup_request_set_key(BuddyLookupRequest *req, const char *key);
...
SEPHONE_PUBLIC unsigned int sip_setup_get_capabilities(SipSetup *s);

SipSetupContext * sip_setup_context_new(SipSetup *s, struct _SephoneProxyConfig *cfg);
...
SEPHONE_PUBLIC int sip_setup_context_get_capabilities(SipSetupContext *ctx);
SEPHONE_PUBLIC int sip_setup_context_login_account(SipSetupContext * ctx, const char *uri, const char *passwd, const char *userid);
...
SEPHONE_PUBLIC BuddyLookupRequest *sip_setup_context_create_buddy_lookup_request(SipSetupContext *ctx);
SEPHONE_PUBLIC int sip_setup_context_buddy_lookup_submit(SipSetupContext *ctx , BuddyLookupRequest *req);
SEPHONE_PUBLIC int sip_setup_context_buddy_lookup_free(SipSetupContext *ctx , BuddyLookupRequest *req);
...
SEPHONE_PUBLIC int sip_setup_context_logout(SipSetupContext *ctx);

/*internal methods for use WITHIN plugins: do not use elsewhere*/
struct _SephoneProxyConfig *sip_setup_context_get_proxy_config(const SipSetupContext *ctx);
